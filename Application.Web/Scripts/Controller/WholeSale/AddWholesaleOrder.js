function getWholesaleBranchList() {
    $.ajax({
        dataType: "json",
...
function getWholesaleItemTypeList() {
    $.ajax({
        dataType: "json",
...
function getWholesaleSupplierList() {
    $.ajax({
        dataType: "json",
...
function getWholesaleCategoryList() {
    $.ajax({
        dataType: "json",
...
function getWholesaleLowStockItemCount() {
    $.ajax({
        dataType: "json",
...
function getWholesaleProductList(pageNo, isRenderPaging, callback) {
        ...
        success: function (data) {
            ...
            if (data.recordList.length > 0) {
                for (var i = 0; i < data.recordList.length; i++) {
                    ...
                    record.push(recordSet.WholesalePrice);
                    //record.push(recordSet.PriceText);
                    //record.push(recordSet.DiscountText);
                    record.push(recordSet.WholesaleDiscount);
...
function getProducts(pageNo, isRenderPaging) {

    $('#dt-wholesale-Products tbody').empty();
    ...
    var isLoad = 'all';
    if (isLoad) {
        getWholesaleProductList(pageNo, isRenderPaging, function (records) {

            //$('#dt-myProducts_wrapper').show();

            $('#dt-wholesale-Products').dataTable({
                "data": records,
                "destroy": true,
                "bLengthChange": false,
                "bFilter": true,
                ...
                "aoColumnDefs": [
                    ...
                    },
                    {
                        "aTargets": [6],
                        "mRender": function (data, type, row) {
                            var text = siteCurrency() + '<input id="btnPrice_' + row[0] + '" style="width:75px; margin-left: 5px;" type="number" class="form-control" value="' + row[6] + '"/>';
                            return $("<div/>").append(text).html();
                        }
                    },
                    {
                        "aTargets": [7],
                        "mRender": function (data, type, row) {
                            var text = '<input id="btnDiscount_' + row[0] + '" style="width:75px;" type="number" class="form-control" value="' + row[7] + '"/>';
                            return $("<div/>").append(text).html();
                        }
                    },
                    {
                    ...
                    //},
                    {
                        ...
                        "mRender": function (data, type, row) {
                            var buttons = '';
                            //'<div><a title="Edit Product" id=' + row[0] + ' class="edit-product cursor-pointer"><b>Edit</b></a></div>';
                            ...
                            buttons += '<div class="ad-action"><a title="Add to cart" gst=' + row[14] + ' imageUrl=' + row[1] + ' name="' + row[3] + '" product-id=' + row[0] + ' id=' + row[0] + ' class="wholesale-order-add-to-cart cursor-pointer">Add to cart</a></div>';
...
app.factory('adminAddWholesaleOrderService', [
    '$http', function ($http) {
...
app.controller('AdminAddWholesaleOrderCtrl', ['$rootScope', '$scope', '$http', '$filter', '$location', 'Enum', 'adminAddWholesaleOrderService', function ($rootScope, $scope, $http, $filter, $location, Enum, adminAddWholesaleOrderService) {

    //$("#txt-search").focus();
    getWholesaleLowStockItemCount();
    getWholesaleBranchList();
    getWholesaleCategoryList();
    getWholesaleSupplierList();
    getWholesaleItemTypeList();
    getProducts(1, true);
    ...
    $('#dt-wholesale-Products').on("click", '.wholesale-order-add-to-cart', function () {

        let orderType = localStorage.getItem("OrderType");
        if (orderType == null || orderType != "Wholesale") {
            localStorage.removeItem("cart");
            localStorage.setItem("OrderType", "Wholesale");
        } else {
            localStorage.setItem("OrderType", "Wholesale");
        }
        ...
        let wholesalePrice = +$("#btnPrice_" + prodId).val();
        let imageUrl = $(this).attr('imageUrl');
        let discount = +$("#btnDiscount_" + prodId).val();

        let gst = $(this).attr('gst');
        addToCart(prodId, name, qty, wholesalePrice, imageUrl, gst, discount);
