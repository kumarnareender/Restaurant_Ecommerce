    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.IsSuccess) {
                ...
                $("#status").html(html);
                populateHistory(statuses, orderId);
                populateOrder();
            }
        }
    });
...
function populateHistory(statuses) {
...
function updateOrderStatus(orderStatusName) {

    let oldStatusId = $("#orderStatusId").val();
    let description = $("#orderDescription").val();

    $.ajax({
        dataType: "json",
        url: '/OrderStatus/CreateOrderStatusHistory',
        data: { OrderId: getParam('orderId'), Description: description, OldStatusId: oldStatusId, NewStatusId: $("#status").val(), NewStatus: orderStatusName },
        success: function (data) {
            if (data.IsSuccess) {

                bootbox.alert("<h4>Successfully updated the status!</h4>", function () { });
                $("#orderDescription").val('');
                populateHistory(statuses);
                populateOrder();
            }
            else {
...
app.controller('AdminOrderDetailsCtrl', ['$rootScope', '$scope', '$http', '$filter', '$location', 'Enum', 'service', function ($rootScope, $scope, $http, $filter, $location, Enum, service) {
    ...
    $('#updateOrderStatus').click(function () {
        updateOrderStatus($('option:selected', "#status").attr('data-id'));
    });
